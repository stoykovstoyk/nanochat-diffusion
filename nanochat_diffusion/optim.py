...
from nanochat_diffusion.common import COMPUTE_DTYPE
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def step(self) -> None:
        """
        Execute a single optimization step across all ranks.

        The step is split into three phases to maximize overlap between
        communication and computation:

        Phase 1: Launch all async reduce ops (reduce for AdamW, reduce_scatter for Muon).
        Phase 2: For each group, wait for its reduce, compute the update, and launch gather.
        Phase 3: Wait for all gathers to complete and copy params back to the correct locations.
        """
        ddp, ddp_rank, ddp_local_rank, ddp_world_size = get_dist_info()
        if not ddp:
            raise RuntimeError("DistMuonAdamW must be used in distributed (DDP) mode.")

        gather_list = []

        # Phase 1: Launch all async reduce ops
        adamw_info = self._reduce_adamw(
            self.param_groups[0], ddp_world_size)
        muon_info = self._reduce_muon(
            self.param_groups[1], ddp_world_size)

        # Phase 2: Compute updates and launch gathers
        # Note: This is a simplified version. In a full implementation,
        # all groups would be processed in parallel here.
        self._compute_adamw(
            self.param_groups[0], adamw_info, gather_list, ddp_rank, ddp_world_size)
        self._compute_muon(
            self.param_groups[1], muon_info, gather_list, ddp_rank)

        # Phase 3: Wait for all gathers and copy back
